...
#   array = []
#   #sub_array = array[p..r] - must find q, thi midpoint

#   midpoint = array.first + array.last / array.length.round
#   puts array / midpoint.round
# end

# puts [4,2,9,8,3].mergesort
...
class MergeSortAlgorithm
  ...
  def sort(numbers)
    if numbers.size <= 1
      return numbers
    end #but this sort method does nothing?

    array_size   = numbers.size
    half_of_size = (array_size / 2).round

    left_array  = numbers.take(half_of_size) #the left array is the first half (lower) of the array
    right_array = numbers.drop(half_of_size) #the right array is the second (higher) half of the array
